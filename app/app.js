...
app.get("/db/getScore", function(req, res){
  ...
  sql.connect(db_config, function(err){
      ...
      for(i=0; i< g_active_manager.length; i++)
     {
          var request = new sql.Request();
          request.stream = true;
          request.query('SELECT * FROM IEF.dbo.ManagerFundHist where Memindex=' + g_active_manager[i] + 'order by TDate');
          var result = {};
...
app.get("/db/getPerfGraph5y", function(req, res){
